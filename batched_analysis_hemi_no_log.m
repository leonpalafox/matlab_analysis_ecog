...
figure
fpc = size(F,1);
%spm=large_power_matrix(:, [1:fpc fpc*2+1:fpc*2+fpc]);
spm=large_power_matrix(:, [1:fpc]);
spm=large_power_matrix;
plot(T_axis, large_power_matrix(:,1))
hold on
plot(T_axis, large_power_matrix(:,2), 'r')
plot(T_axis, large_power_matrix(:,3), 'g')
plot(T_axis, large_power_matrix(:,4), 'c')
pc=pca(spm);
spm_pc = spm*pc(:,1:5);
plot(T_axis, spm_pc(:,1), 'LineWidth', 3,'Color' ,'k')
%[b,dev,stats] = glmfit(spm_pc ,[large_labels ones(size(large_labels))],'binomial','link','logit'); % Logistic regression
spm_pc = large_power_matrix;
[b,dev,stats] = glmfit(spm_pc ,large_labels); % Logistic regression
